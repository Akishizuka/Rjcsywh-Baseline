...
def Trainer(model, model_optimizer, train_dl, val_dl, test_dl, device, config, idx):
    ...
    # Window-level ROC-AUC: score is softmax probability of class1; label is window label from dataloader
    test_roc_auc = float("nan")
    y_true = np.asarray(test_target).reshape(-1).astype(int)
    y_score = np.asarray(test_score_origin).reshape(-1).astype(float)
    if len(np.unique(y_true)) < 2:
        print("Test ROC-AUC: undefined (only one class in test windows)\n")
    else:
        try:
            test_roc_auc = float(roc_auc_score(y_true, y_score))
            print(f"Test ROC-AUC: {test_roc_auc:.5f}\n")
        except ValueError:
            print("Test ROC-AUC: undefined (sklearn could not compute)\n")
